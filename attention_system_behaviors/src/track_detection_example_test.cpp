...
main(int argc, char* argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("track_detection_example_test_node");
  ...
  //factory.registerFromPlugin(loader.getOSName("check_string_topic_bt_node"));

  std::string pkgpath = ament_index_cpp::get_package_share_directory("attention_system_behaviors");
  std::string xml_file = pkgpath + "/behavior_tree_xml/track_detection_example.xml";

  auto blackboard = BT::Blackboard::create();
  blackboard->set<rclcpp::Node::SharedPtr>("node", node);

  blackboard->set<int>("retry_attempts", 5);
  blackboard->set<std::string>("det_class", "person's face");
