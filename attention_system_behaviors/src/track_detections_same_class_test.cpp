...
main(int argc, char* argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("track_detections_same_class_midpoint_rot_test_node");
  ...
  factory.registerFromPlugin(loader.getOSName("request_n_detections_same_class_track_point_bt_node"));

  std::string pkgpath = ament_index_cpp::get_package_share_directory("attention_system_behaviors");
  std::string xml_file = pkgpath + "/behavior_tree_xml/track_detections_same_class_midpoint_rot.xml";

  auto blackboard = BT::Blackboard::create();
  blackboard->set<rclcpp::Node::SharedPtr>("node", node);

  blackboard->set<std::string>("det_class", "Person");
  blackboard->set<int>("n_detections", -1);
