...
Detection3dProjectorAstraDepht::Detection3dProjectorAstraDepht()
: Node("spatial_projector_astra_depht_node"),
  camera_info_received_(false),
  depth_camera_topic_(""),
  depth_to_color_extrinsics_topic_("/extrinsic/depth_to_color"),
  ...
  extrinsics_translation_scale_(0.001),
  previous_marker_count_(0)
{
  this->declare_parameter<std::string>("depth_camera_topic", "");
  this->declare_parameter<std::string>(
    "depth_to_color_extrinsics_topic", "/extrinsic/depth_to_color");
  ...
  this->declare_parameter<double>("extrinsics_translation_scale", 0.001);

  this->get_parameter("depth_camera_topic", depth_camera_topic_);
  this->get_parameter("depth_to_color_extrinsics_topic", depth_to_color_extrinsics_topic_);
  ...
  this->get_parameter("extrinsics_translation_scale", extrinsics_translation_scale_);
  ...
  // TODO hay que refactorizar esto que esta feísimo

  if (!depth_camera_topic_.empty()) {
    auto extrinsics_qos = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable();

    extrinsics_sub_ = this->create_subscription<astra_camera_msgs::msg::Extrinsics>(
      depth_to_color_extrinsics_topic_, extrinsics_qos,
      std::bind(&Detection3dProjectorAstraDepht::extrinsics_callback, this, std::placeholders::_1));

    RCLCPP_INFO(
      this->get_logger(),
      "Using depth to color extrinsics topic: %s",
  ...
  if (depth_camera_topic_.empty()) {
    RCLCPP_WARN(
      this->get_logger(),
      "depth_camera_topic is empty. Astra depth projector will skip detections.");
  } else if (depth_to_color_extrinsics_topic_.empty()) {
    RCLCPP_ERROR(
      this->get_logger(),
      "depth_to_color_extrinsics_topic is mandatory when depth_camera_topic is configured.");
    throw std::runtime_error(
      "depth_to_color_extrinsics_topic is mandatory when depth_camera_topic is configured");
  } else {
    depth_image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      depth_camera_topic_, rclcpp::SensorDataQoS(),
      std::bind(&Detection3dProjectorAstraDepht::depth_image_callback, this, std::placeholders::_1));
  ...
  cam_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
    ...
    std::bind(&Detection3dProjectorAstraDepht::camera_info_callback, this, std::placeholders::_1));

  RCLCPP_INFO(
    this->get_logger(),
    "Spatial Projector Astra Depht Node initialized. Waiting for CameraInfo...");
}

void
Detection3dProjectorAstraDepht::depth_image_callback(
...
void
Detection3dProjectorAstraDepht::extrinsics_callback(
  const astra_camera_msgs::msg::Extrinsics::SharedPtr msg)
{
  last_depth_to_color_extrinsics_ = msg;
}

void
Detection3dProjectorAstraDepht::camera_info_callback(
  const sensor_msgs::msg::CameraInfo::SharedPtr msg)
{
  if (!camera_info_received_) {
    ...
    det_sub_ = this->create_subscription<vision_msgs::msg::Detection2DArray>(
      "detections_2d", 10,
      std::bind(&Detection3dProjectorAstraDepht::detections_callback, this, std::placeholders::_1));
  }
}

bool
Detection3dProjectorAstraDepht::get_depth_from_image(
  double u,
  double v,
...
bool
Detection3dProjectorAstraDepht::get_depth_from_32fc1(
...
bool
Detection3dProjectorAstraDepht::get_depth_from_16uc1(
  const sensor_msgs::msg::Image & depth_image,
  int u,
  int v,
  double & z_meters) const
...
Detection3dProjectorAstraDepht::create_centroid_marker(
...
bool
Detection3dProjectorAstraDepht::transform_depth_to_color(
  double & x_meters,
  double & y_meters,
  double & z_meters) const
{
  if (!last_depth_to_color_extrinsics_) {
    RCLCPP_WARN_THROTTLE(
      this->get_logger(),
      *this->get_clock(),
      2000,
      "Detections received, but waiting for depth to color extrinsics...");
    return false;
  }

  const auto & extrinsics = *last_depth_to_color_extrinsics_;
  const double translation_x = extrinsics.translation[0] * extrinsics_translation_scale_;
  const double translation_y = extrinsics.translation[1] * extrinsics_translation_scale_;
  const double translation_z = extrinsics.translation[2] * extrinsics_translation_scale_;

  const double transformed_x = (extrinsics.rotation[0] * x_meters) +
    (extrinsics.rotation[1] * y_meters) +
    (extrinsics.rotation[2] * z_meters) +
    translation_x;
  const double transformed_y = (extrinsics.rotation[3] * x_meters) +
    (extrinsics.rotation[4] * y_meters) +
    (extrinsics.rotation[5] * z_meters) +
    translation_y;
  const double transformed_z = (extrinsics.rotation[6] * x_meters) +
    (extrinsics.rotation[7] * y_meters) +
    (extrinsics.rotation[8] * z_meters) +
    translation_z;

  if (
    !std::isfinite(transformed_x) ||
  ...
  y_meters = transformed_y;
  z_meters = transformed_z;

  return true;
}

void
Detection3dProjectorAstraDepht::detections_callback(
  const vision_msgs::msg::Detection2DArray::SharedPtr msg)
{
  ...
  for (const auto & det2d : msg->detections) {
    ...
    double optical_x_meters = normalized_x * z_meters;
    double optical_y_meters = normalized_y * z_meters;
    double optical_z_meters = z_meters;

    if (!transform_depth_to_color(optical_x_meters, optical_y_meters, optical_z_meters)) {
      continue;
    }
