#ifndef ATTENTION_AUX_PERCEPTION_NODES__PUB_N_DETECTIONS_SAME_CLASS_TF__HPP
#define ATTENTION_AUX_PERCEPTION_NODES__PUB_N_DETECTIONS_SAME_CLASS_TF__HPP
...
class PubNDetectionsSameClassTF : public rclcpp::Node
{

public:
  PubNDetectionsSameClassTF();
  virtual ~PubNDetectionsSameClassTF() = default;
...
private:
  ...
  bool update_tracked_tf(
    const std::vector<const vision_msgs::msg::Detection3D *> & tracked_detections);
  void publish_tracked_detection_ids();
  void publish_detection_tf(std::string child_frame_id);

  int8_t n_detections_;
  std::string detection_class_name_;
  std::string frame_id_;
  std::string tracked_detection_ids_topic_;
  std::string detection_point_topic_;
  int tf_lookup_timeout_ms_;
  std::vector<std::string> tracked_detection_ids_;
  std::unordered_map<std::string, int> missing_iterations_by_id_;
  rclcpp::Publisher<vision_msgs::msg::Detection2DArray>::SharedPtr tracked_ids_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PointStamped>::SharedPtr detection_point_pub_;
  const int max_missing_iterations_ = 5;
  ...
  rclcpp::Service<attention_aux_perception_msgs::srv::StartNDetectionsSameClassTFPub>::SharedPtr start_pub_srv_;
...
#endif // ATTENTION_AUX_PERCEPTION_NODES__PUB_N_DETECTIONS_SAME_CLASS_TF__HPP
